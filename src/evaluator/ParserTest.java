...
public class ParserTest {
    ...
    public void testProgram() {
        ...
        equalIgnoreTypes(Tree.parse("def(foo $seq(x y) $seq(+(x y))"), parser.functions.get("foo"));

        use("def three = 1, 2, 3 end");
        parser.program();
        assertEquals(0, parser.stack.size());
        equalIgnoreTypes(Tree.parse("def(three $seq $seq(1.0 2.0 3.0)"), parser.functions.get("three"));
        equalIgnoreTypes(Tree.parse("def(foo $seq(x y) $seq(+(x y))"), parser.functions.get("foo"));
    }
...
//        assertEquals(expectedTree, actualTree);
        ...
        assertEquals(1, actualTree.numberOfChildren());
        assertEquals("quoted string", actualTree.firstChild().getValue().getValue());
    }
    ...
    private boolean equalIgnoreTypes(Tree<String> tree1, Tree<Token> tree2) {
        ...
        if (tree1.numberOfChildren() != tree2.numberOfChildren()) return false;
        for (int i = 0; i < tree1.numberOfChildren(); i++) {
            if (! equalIgnoreTypes(tree1.child(i), tree2.child(i))) return false;
        }
        return true;
    }
